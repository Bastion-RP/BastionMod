class BastionVendor extends DayZPlayer
{
	...
	void OnInitServer()
	{
		TraderPlayerSettings traderSettings = GetServerSettings().GetTraderSettings().GetTraderByName( GetType() );
        if (traderSettings)
        {
            for ( int i = 0; i < traderSettings.Loadout.Count(); i++ )
		    {
                GetInventory().CreateInInventory( traderSettings.Loadout.Get( i ) );
            }
        }
	}
