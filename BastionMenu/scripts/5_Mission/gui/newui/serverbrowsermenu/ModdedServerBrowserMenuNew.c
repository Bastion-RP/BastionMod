modded class ServerBrowserMenuNew extends UIScriptedMenu
{
	...
	protected Widget m_Server_1;
	protected Widget m_Server_2;

	protected TextWidget m_Server_1_text;
	protected TextWidget m_Server_2_text;
	
	override Widget Init()
	{
#ifdef PLATFORM_CONSOLE
		layoutRoot = GetGame().GetWorkspace().CreateWidgets( "BastionMod/BastionMenu/gui/layouts/new_ui/server_browser/pc/server_browser.layout" );
		...
		m_Server_1 = layoutRoot.FindAnyWidget( "server_1" );
		m_Server_2 = layoutRoot.FindAnyWidget( "server_2" );
#endif
	...
	override bool OnClick( Widget w, int x, int y, int button )
	{
		if( button == MouseState.LEFT )
		{
			...
			if( w == m_Server_1 )
			{
				g_Game.ConnectFromServerBrowser( "51.81.48.230", 2302, "" ); //Stable Server
				return true;
			}
			else if ( w == m_Server_2 )
			{
				g_Game.ConnectFromServerBrowser( "38.65.5.145", 2302, "noelcoco" ); //Experimental Server
				return true;
			}
		}
		return false;
