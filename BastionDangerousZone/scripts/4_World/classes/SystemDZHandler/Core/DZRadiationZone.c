	...
	void HighLevelRadiationZoneRound(int m_NbSickGivenForRadiation,float m_X, float m_Y, float Zone_Radius,string Status,PlayerBase player)
	{
		...
		if ( distance_squared <= Math.Pow(0.33*Zone_Radius, 2) )
		{
			if (player.IsInside.DZName==Status && player.IsInside.DZStatut && player.NbSickGivenForRadiation == m_NbSickGivenForRadiation*3) //Already in zone
			{
				player.RadCheck();
				#ifdef DZDEBUG
				GetDZLogger().LogInfo(player.GetIdentity().GetName() + "is inside CENTER:" + player.IsInside.DZStatut.ToString()+ "radsickpoint given :" + player.NbSickGivenForRadiation.ToString());
				#endif
				return;
			}
			else
		...
		else 	if ( distance_squared <= Math.Pow(0.66*Zone_Radius, 2) )
			{
				if (player.IsInside.DZName==Status && player.IsInside.DZStatut && player.NbSickGivenForRadiation == m_NbSickGivenForRadiation*2) //Already in zone
				{
					player.RadCheck();
					#ifdef DZDEBUG
					GetDZLogger().LogInfo(player.GetIdentity().GetName() + "is inside INTERMEDIATE:" + player.IsInside.DZStatut.ToString() + "radsickpoint given :" + player.NbSickGivenForRadiation.ToString());
					#endif
					return;
				}
				else
			...
			else 	if ( distance_squared <= Math.Pow(Zone_Radius, 2) )
			{
					...
					{
						#ifdef DZDEBUG
						GetDZLogger().LogInfo(player.GetIdentity().GetName() + "is inside BORDER:" + player.IsInside.DZStatut.ToString() + "radsickpoint given :" + player.NbSickGivenForRadiation.ToString());
						#endif
						player.RadCheck();
						return;
					}
