class ActionDismantleBase_CornerSofaCB : ActionContinuousBaseCB
{
...
class ActionDismantleBase_CornerSofa: ActionContinuousBase
{
	void ActionDismantleBase_CornerSofa()
	{
		m_CallbackClass = ActionDismantleBase_CornerSofaCB;
	...
	{	
		...
		if ( targetObject )
		{
			Base_CornerSofa base_cornersofa = Base_CornerSofa.Cast( targetObject );
			if ( base_cornersofa )
			{
				return true;
			}
	...
	override void OnFinishProgressServer( ActionData action_data ) 
	{
		...
		Base_CornerSofa base_cornersofa = Base_CornerSofa.Cast( action_data.m_Target.GetObject() );
		
		if ( base_cornersofa ) 
		{	
			//delete existing model
			base_cornersofa.Base_Destroy();
			...
			ItemBase Remaining_Planks = ItemBase.Cast( GetGame().CreateObject("Base_CornerSofa_Kit", pos) );
			int plank_quant = 1;
		...
		action_data.m_Player.MessageStatus((base_cornersofa.GetPosition()-action_data.m_Player.GetPosition()).ToString());
