class ActionDismantleBase_Chernarus_FlagCB : ActionContinuousBaseCB
{
...
class ActionDismantleBase_Chernarus_Flag: ActionContinuousBase
{
	...
	{	
		...
		if ( targetObject )
		{
			Base_Chernarus_Flag base_chernarus_flag = Base_Chernarus_Flag.Cast( targetObject );
			if ( base_chernarus_flag )
			{
				return true;
			}
		}
	...
	override void OnFinishProgressServer( ActionData action_data ) 
	{
		...
		Base_Chernarus_Flag base_chernarus_flag = Base_Chernarus_Flag.Cast( action_data.m_Target.GetObject() );
		...
		{	
			...
			ItemBase Remaining_Planks = ItemBase.Cast( GetGame().CreateObject("Base_Chernarus_Flag_Kit", pos) );
			int plank_quant = 1;
		...
		action_data.m_Player.MessageStatus((base_chernarus_flag.GetPosition()-action_data.m_Player.GetPosition()).ToString());
