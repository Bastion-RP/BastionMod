class ActionDismantleBase_BMetalCrateCB : ActionContinuousBaseCB
{
...
class ActionDismantleBase_BMetalCrate: ActionContinuousBase
{
	void ActionDismantleBase_BMetalCrate()
	{
		m_CallbackClass = ActionDismantleBase_BMetalCrateCB;
	...
	override string GetText()
	{
		...
		if ( player )
		{
			return "Dismantle Blue Metal Crate";
		}
	...
	{	
		...
		if ( targetObject )
		{
			Base_BMetalCrate base_bmetalcrate = Base_BMetalCrate.Cast( targetObject );
			if ( base_bmetalcrate )
			{
				if ( base_bmetalcrate.GetInventory().GetCargo().GetItemCount() == 0 )
				{
					return true;
				}
			}
	...
	override void OnFinishProgressServer( ActionData action_data ) 
	{
		...
		Base_BMetalCrate base_bmetalcrate = Base_BMetalCrate.Cast( action_data.m_Target.GetObject() );
		
		if ( base_bmetalcrate ) 
		{	
			//delete existing model
			base_bmetalcrate.Base_Destroy();
			...
			ItemBase Remaining_Planks = ItemBase.Cast( GetGame().CreateObject("Base_BMetalCrate_Kit", pos) );
			int plank_quant = 1;
		...
		action_data.m_Player.MessageStatus((base_bmetalcrate.GetPosition()-action_data.m_Player.GetPosition()).ToString());
