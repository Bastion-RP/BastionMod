class ActionDismantleBase_GMetalCrateCB : ActionContinuousBaseCB
{
...
class ActionDismantleBase_GMetalCrate: ActionContinuousBase
{
	...
	override string GetText()
	{
		...
		if ( player )
		{
			return "Dismantle Green Metal Crate";
		}
	...
	{	
		...
		if ( targetObject )
		{
			Base_GMetalCrate base_gmetalcrate = Base_GMetalCrate.Cast( targetObject );
			if ( base_gmetalcrate )
			{
				if ( base_gmetalcrate.GetInventory().GetCargo().GetItemCount() == 0 )
				{
					return true;
				}
			}
	...
	override void OnFinishProgressServer( ActionData action_data ) 
	{
		...
		Base_GMetalCrate base_gmetalcrate = Base_GMetalCrate.Cast( action_data.m_Target.GetObject() );
		
		if ( base_gmetalcrate ) 
		{	
			//delete existing model
			base_gmetalcrate.Base_Destroy();
			...
			ItemBase Remaining_Planks = ItemBase.Cast( GetGame().CreateObject("Base_GMetalCrate_Kit", pos) );
			int plank_quant = 1;
		...
		action_data.m_Player.MessageStatus((base_gmetalcrate.GetPosition()-action_data.m_Player.GetPosition()).ToString());
