class ActionDismantleBase_NightstandCB : ActionContinuousBaseCB
{
...
class ActionDismantleBase_Nightstand: ActionContinuousBase
{
	...
	{	
		...
		if ( targetObject )
		{
			Base_Nightstand base_nightstand = Base_Nightstand.Cast( targetObject );
			if ( base_nightstand )
			{
				if ( base_nightstand.GetInventory().GetCargo().GetItemCount() == 0 )
				{
					return true;
				}
			}
	...
	override void OnFinishProgressServer( ActionData action_data ) 
	{
		...
		Base_Nightstand base_nightstand = Base_Nightstand.Cast( action_data.m_Target.GetObject() );
		...
		{	
			...
			ItemBase Remaining_Planks = ItemBase.Cast( GetGame().CreateObject("Base_Nightstand_Kit", pos) );
			int plank_quant = 1;
		...
		action_data.m_Player.MessageStatus((base_nightstand.GetPosition()-action_data.m_Player.GetPosition()).ToString());
