class ActionDismantleBase_CotCB : ActionContinuousBaseCB
{
...
class ActionDismantleBase_Cot: ActionContinuousBase
{
	void ActionDismantleBase_Cot()
	{
		m_CallbackClass = ActionDismantleBase_CotCB;
	...
	{	
		...
		if ( targetObject )
		{
			Base_Cot base_cot = Base_Cot.Cast( targetObject );
			if ( base_cot )
			{
				if ( base_cot.GetInventory().GetCargo().GetItemCount() == 0 && base_cot.GetInventory().AttachmentCount() == 0 )
				{
					return true;
				}
			}
	...
	override void OnFinishProgressServer( ActionData action_data ) 
	{
		...
		Base_Cot base_cot = Base_Cot.Cast( action_data.m_Target.GetObject() );
		
		if ( base_cot ) 
		{	
			//delete existing model
			base_cot.Base_Destroy();
			...
			ItemBase Remaining_Planks = ItemBase.Cast( GetGame().CreateObject("Base_Cot_Kit", pos) );
			int plank_quant = 1;
		...
		action_data.m_Player.MessageStatus((base_cot.GetPosition()-action_data.m_Player.GetPosition()).ToString());
