class ActionDismantleBase_SingleBedCB : ActionContinuousBaseCB
{
...
class ActionDismantleBase_SingleBed: ActionContinuousBase
{
	void ActionDismantleBase_SingleBed()
	{
		m_CallbackClass = ActionDismantleBase_SingleBedCB;
	...
	{	
		...
		if ( targetObject )
		{
			Base_SingleBed base_singlebed = Base_SingleBed.Cast( targetObject );
			if ( base_singlebed )
			{
				if ( base_singlebed.GetInventory().GetCargo().GetItemCount() == 0 && base_singlebed.GetInventory().AttachmentCount() == 0 )
				{
					return true;
				}
			}
	...
	override void OnFinishProgressServer( ActionData action_data ) 
	{
		...
		Base_SingleBed base_singlebed = Base_SingleBed.Cast( action_data.m_Target.GetObject() );
		
		if ( base_singlebed ) 
		{	
			//delete existing model
			base_singlebed.Base_Destroy();
			...
			ItemBase Remaining_Planks = ItemBase.Cast( GetGame().CreateObject("Base_SingleBed_Kit", pos) );
			int plank_quant = 1;
		...
		action_data.m_Player.MessageStatus((base_singlebed.GetPosition()-action_data.m_Player.GetPosition()).ToString());
