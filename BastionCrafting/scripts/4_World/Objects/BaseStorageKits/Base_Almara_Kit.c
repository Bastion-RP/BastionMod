class Base_Almara_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								Base_Almara_Kit1;
	
	void Base_Almara_Kit()
	{
	...
	override void OnPlacementComplete( Man player )
	{
		if ( GetGame().IsServer() )
		{
			...
			EntityAI item = EntityAI.Cast(GetGame().CreateObjectEx("Base_Almara", position, ECE_PLACE_ON_SURFACE ));
...
class Base_Almara_Holo extends ItemBase
{
...
class Base_Almara extends ItemBase
{
		override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
