class Base_BMetalCrate_Kit extends ItemBase
{	
	ref protected EffectSound 						m_DeployLoopSound;
	protected Object								Base_BMetalCrate_Kit1;
	
	void Base_BMetalCrate_Kit()
	{
	...
	override void OnPlacementComplete( Man player, vector position = "0 0 0", vector orientation = "0 0 0" )
	{
		if ( GetGame().IsServer() )
		{
			EntityAI item = EntityAI.Cast(GetGame().CreateObjectEx("Base_BMetalCrate", position, ECE_PLACE_ON_SURFACE ));
...
class Base_BMetalCrate_Holo extends ItemBase
{
...
class Base_BMetalCrate extends ItemBase
{
		override bool CanPutInCargo( EntityAI parent )
    {
        if( !super.CanPutInCargo(parent) ) {return false;}        
